...
if [[ -n "${COPILOT_CLI_VERSION_OVERRIDE:-}" ]]; then
  latest_tag="${COPILOT_CLI_VERSION_OVERRIDE#v}"
  echo "Using override version: v${latest_tag}"
else
  echo "Fetching latest release from github/copilot-cli..."
  latest_tag=$(
    git ls-remote --tags --refs https://github.com/github/copilot-cli.git \
...
for system, new_hash in hashes.items():
    pattern = r'(%s = \{\s*name = "[^"]*";\s*hash = ")([^"]*)(";)' % re.escape(system)
    text, count = re.subn(
        pattern,
