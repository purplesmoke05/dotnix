...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch =
    ...
    + ''
      substituteInPlace packages/opencode/script/build.ts \
        ...
        'if (false)'
    ''
    # Disable code splitting: Bun 1.4.x emits broken compiled bundles with splitting enabled,
    # crashing every prompt in SystemPrompt.environment (anomalyco/opencode #48965, #49262).
    + ''
      substituteInPlace packages/opencode/script/build.ts \
  ...
  installPhase = ''
    ...
    --run '
      ...
      # Keep using the legacy nixpkgs OpenCode database when it exists so
      # existing sessions remain visible after the channel switch.
